...
export function solve (input: string) {
  ...
  for (let i = 1; i <= NGarbagesCount; i++) {
    ...
    if (rowGarbages.has(Xi)) {
      rowGarbages.get(Xi).add(Yi)
    } else {
      rowGarbages.set(Xi, new Set<number>())
      rowGarbages.get(Xi).add(Yi)
    }

    if (colGarbages.has(Yi)) {
      colGarbages.get(Yi).add(Xi)
    } else {
      colGarbages.set(Yi, new Set<number>())
      colGarbages.get(Yi).add(Xi)
    }
  }
  ...
  for (let j = NGarbagesCount + 2; j < lines.length; j++) {
    ...
    if (queryType === 1) {
      if (
        !rowGarbages.has(targetPosition) ||
        rowGarbages.get(targetPosition)?.size < 1
      ) {
        console.log(0)
        continue
      }
      ...
      if (
        !colGarbages.has(targetPosition) ||
        colGarbages.get(targetPosition)?.size < 1
      ) {
        console.log(0)
        continue
      }
