...
漸化式は、数列の各項を前の項（または前のいくつかの項）を使って表現する式のことです。「前の値を使って次の値を定義する関係式」と考えるとわかりやすいでしょう。

基本的な考え方

漸化式は以下の 2 つの要素で構成されます。

1. 初期条件：最初の値（スタート地点）
2. 漸化関係：前の項から次の項を求める規則
...
- 初期条件：F(0),F(1)=1
- 漸化式：F(n)=F(n-1) + F(n-2) ( n≥2)

これにより、0,1,1,2,3,5,8,13,21...といいう数列が生まれる

### トーラス構造とは

端と端がつながっている構造。ゲーム「マリオブラザーズ」のマップのように、右端を通ると左端から戻ってくるような構造のこと。
...
上記の `+N) % N` の部分がポイントである。
この問題では、トーラス構造になっており、移動によって端を超えると、反対側の端から戻ってくる構造になっている。
`+ N`をしておくことで、移動結果がマイナス（具体的には-1）がなるときにおいて、マイナスの数値に対して余りを求めないようにする。
余りを求めれば、トーラス構造として元の位置に戻ってきたときの位置を求めることができる。
